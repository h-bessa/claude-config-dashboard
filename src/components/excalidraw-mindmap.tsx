...
export function ExcalidrawMindmap({ data }: { data: ConfigData }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      // Build a simple test first to verify convertToExcalidrawElements works
      const testElements = convertToExcalidrawElements([
        {
          type: "rectangle",
          x: 0,
          y: 0,
      ...
      console.log("[excalidraw] test elements:", testElements.length, testElements[0]?.type);

      if (testElements.length === 0) {
        console.error("[excalidraw] convertToExcalidrawElements returned empty array for test");
        return;
      }

      // Now build the full mindmap
      const skeleton: any[] = [];
      ...
      branches.forEach((b) => {
        ...
        b.items.slice(0, 8).forEach((item, i) => {
          ...
          skeleton.push({
            type: "line",
            x: isLeft ? b.x : b.x + 180,
            y: b.y + 20,
            strokeColor: b.color + "33",
            strokeWidth: 1,
            roughness: 0,
            opacity: 30,
        ...
        if (b.items.length > 8) {
          const ix = isLeft ? b.x - 170 : b.x + 200;
          const iy = b.y - 10 + 8 * 32;
          skeleton.push({
            type: "text",
            x: ix + 20,
            y: iy + 5,
            text: `+${b.items.length - 8} more...`,
            fontSize: 10,
            strokeColor: "#666",
          });
        }
      ...
      console.log("[excalidraw] skeleton count:", skeleton.length);
      const elements = convertToExcalidrawElements(skeleton);
      console.log("[excalidraw] converted count:", elements.length);
      ...
      requestAnimationFrame(() => {
        setTimeout(() => {
          ...
          console.log("[excalidraw] scene elements:", sceneElements.length);
